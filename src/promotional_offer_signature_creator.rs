...
impl PromotionalOfferSignatureCreator {
    ...
    pub fn new(private_key: &str, key_id: String, bundle_id: String) -> Result<Self, PromotionalOfferSignatureCreatorError> {
    ...
    pub fn create_signature(
        ...
        let payload = self.payload(product_identifier, subscription_offer_id, application_username, nonce, timestamp);
    ...
    fn sign(&self, payload: &str) -> Result<Signature, PromotionalOfferSignatureCreatorError> {
        Ok(self.ec_private_key.sign(&ring::rand::SystemRandom::new(),payload.as_bytes())?)
    }
...
mod tests {
    ...
    fn test_promotional_offer_signature_creator() {
        ...
        let r = creator.create_signature("com.test.product", "com.test.offer", uuid::Uuid::new_v4().to_string().as_str(), &uuid::Uuid::new_v4(), i64::try_from(system_timestamp()).unwrap()).unwrap();
    ...
    fn test_promotional_offer_signature_creator_verified() {
        let private_key = include_str!("../assets/SubscriptionKey_L256SYR32L.p8");
        let creator = PromotionalOfferSignatureCreator::new(private_key, "L256SYR32L".to_string(), "com.test.app".to_string()).unwrap();
        let payload = creator.payload("com.test.product", "com.test.offer", uuid::Uuid::new_v4().to_string().as_str(), &uuid::Uuid::new_v4(), i64::try_from(system_timestamp()).unwrap());
